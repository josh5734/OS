...
* Software-only solution은 소프트웨어로만 lock을 구현하는 것이다. 아래 가장 기본적인 코드를 보자.
```
[1]
repeat:
...
until false;
```
* 이 방법에서는 여러가지 문제점이 있다. 
    - 우선 자신의 turn이 아닌 녀석들은 계속 while문에 갇혀서 spinlock이 걸린다.
    - Mutual exclusion은 만족하지만 내 턴이 오기 전에 다른 프로세스가 죽어버리면 나한테 턴을 안 넘겨주기 때문에 Progress 조건이 만족되지 않는다.
* 그렇다면 조금 더 발전된 두 번째 방법을 보자.
```
[2]
repeat:
    // flag는 boolean값을 가진 공유변수이며 flag[i] == true라는 것은 Process i가 CS에 진입할 준비가 됐다는 것
    flag[i] := true;    // 맨 처음에 false로 초기화되어 있던 걸 Process i가 들어가겠다고 선언
    while flag[j] do no-ops;    // j는 아직 들어가겠다고 안하면 spinlock인데, j도 들어가겠다고 하면? - Progress 위배
        critical section
    flag[i] := false;
        remainder section
until false;
```
* 이 방법에도 문제가 있다.
    - spinlock 문제는 해결되지 않는다.
    - flag[j] = false라는 것을 보고 다른 사람이 안쓰는구나해서 CS에 진입할 수 있는데, 만약 Process i, j가 동시에 entry section에 진입하면 flag가 둘 다 true되서 교착상태에 빠진다. 즉, Progress 조건이 만족되지 않는 것이다.

* 그렇다면 위 문제들을 해결하려면 어떻게 해야할까? **Peterson's algorithm**을 보자.
```
...
* Hardware Atomic solution은 위처럼 software 코드만으로는 동기화 문제를 해결하기에 너무 느리고 복잡하니까(프로세스가 엄청 많다고 하면 코드가 터지지 않을까?) 고안된 것이다.
...
* 위에서 공부한 방식의 문제점을 해결하려면 어떻게 해야 할까?
* **가장 단순한 방법은 원인을 제거해버리는 것이다. 동기화 이슈의 기본적인 원인은 Interrupt였으니까 Interrupt 자체를 막아버리는 것이다.**
* 하지만, 이렇게 하면 정말 중요한 Interrupt도 막아버리는 문제점이 생긴다...

---


## 3. High-level Synchronization
* 위에서 살펴본 Software-only solution은 복잡성과 spinlock으로 인한 resource의 낭비가 심하기 때문에 효율적이지 않았다. 그리고 무작정 Interrupt를 막아버리는 것도 다소 무리가 있었다.
* 따라서 조금 더 High-level solution이 필요하게 되었는데, High-level Synchronization은 크게 두 가지를 목표로 한다.
```
...
---
### 3) Monitor
* Monitor는 한번에 한 명만 들어올 수 있는 방이라고 이해할 수 있으며 Shared data, Shared data에 대한 Procedure, 그리고 concurrent한 processes에 대한 Synchronization으로 구성된다.
<img src = "https://user-images.githubusercontent.com/61929745/115403613-8c86c980-a227-11eb-925b-2f2e8c640151.png" width = 600 heigth = 600>   

```
- Monitor에는 Condition Queue가 존재하는데 이를 "condition c" 이런 식으로 정의해준다. 어떤 프로세스가 모니터를 수행하다가 이 조건을 만족하지 못하면 Condition queue에서 기다리게 된다.
- Monitor는 한번에 하나의 프로세스만 처리하여 Mutual exclusion을 만족시키며, 만약 다른 프로세스가 monitor procedure에 들어오려고 하면 처음 프로세스가 monitor에서 나갈때까지 block시킨다.
- wait(c) operation은 monitor의 lock을 release해줌으로써 다른 프로세스가 모니터로 들어오게 한다.
- signal(c) operation은 wait하고 있는 프로세스를 깨울 때 사용한다. 만약 waiting process가 없으면 signal은 사라진다.
```
* Producer, consumber 예제에서 쓴 Bounded buffer를 Monitor로 구현해보면 아래와 같다.
```
...
```
* 위의 Monitor 구조 그림과 코드, 주석을 보면 어느정도 이해할 수 있을 것 같다.
---

* 그러면 Synchronization을 pthread에서는 어떻게 처리하는지 다음 글에서 정리하겠다.
