...
## 3. 멀티쓰레딩의 장단점
### 1) 장점
- 쓰레드를 만드는 데 시간이 적게 걸린다.
- 자원을 공유하기 때문에 쓰레드 간 Context Switching을 하는 데 시간이 적게 걸린다. 
### 2) 단점
...
## 4. Process VS Thread
* 공통점
1) Process, Thread 모두 logical control flow가 존재한다.
2) Process, Thread 모두 개별적으로 스케줄링이 가능하다.
* 차이점
1) 쓰레드는 공유되는 코드와 자원(Stack, PC)이 존재하지만, Process는 공유하는 자원이 없다.
2) 멀티쓰레드는 쓰레드의 생성, 스위칭, 종료에 따른 시간과 메모리의 효율이 멀티프로세싱보다 유리하다.
---

* 이제 Multithreading을 실제로 어떻게 구현할 것인지 다음 글에서 공부해보자.
